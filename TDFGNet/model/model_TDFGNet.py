...
class TDFGNet(nn.Module):
    def __init__(self, input_channels, block=ResNet):
        super(TDFGNet, self).__init__()
...
class TDFGNet_DTUM(nn.Module):
    def __init__(self, input_channels, num_classes):
        super(TDFGNet_DTUM, self).__init__()

        self.UNet = TDFGNet(input_channels)#这里的32实际上是输出通道数而非num_class
...
from models.model_TDFGNet import TDFGNet_DTUM  # 确保路径正确
...
# ----------------- 构造模型 -----------------
model = TDFGNet_DTUM(input_channels=C, num_classes=num_classes).to(device)
model.eval()
