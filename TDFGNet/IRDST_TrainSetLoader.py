...
class IRDST_TrainDataset(torch.utils.data.Dataset):
    def __init__(self, num_frames=7, train_size=256):
        self.num_frames = num_frames
        self.train_size = train_size
                                
        self.train_seqs_file = '/home/tcs1/data2/lyc/DTUM_main/dataset/IRDST/IRDST/train.txt'
    ...
    def __getitem__(self, idx):
        ...
        imgs = torch.from_numpy(np.expand_dims(imgs, axis=0))  # [1, num_frames, H, W]
        ...
        mask = cv2.resize(mask, (self.train_size, self.train_size), interpolation=cv2.INTER_NEAREST)
        mask[mask > 0] = 1
        mask = torch.from_numpy(np.expand_dims(np.expand_dims(mask.astype(np.float32), 0), 0))  # [1,1,H,W]

        m, n = mask.shape[2], mask.shape[3]
